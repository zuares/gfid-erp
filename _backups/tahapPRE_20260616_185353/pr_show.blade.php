...
<div class="pr-show-wrap">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 gap-2 flex-wrap">
        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            {{-- PR-D: Convert → PO — hanya jika approved dan belum pernah convert --}}
    ...
    {{-- PR-D: Info PO hasil convert --}}
    @if ($pr->isConverted() && $pr->converted_to_po_id)
        <div class="pr-info-card" style="border-color: rgba(37,99,235,.3);">
            <div class="card-body">
                <div class="d-flex align-items-center gap-2">
                    <span style="font-size:1.1rem;">🔗</span>
                    <div>
                        <div class="pr-info-label mb-0">Sudah diconvert ke Purchase Order</div>
                        @if (\Illuminate\Support\Facades\Route::has('purchasing.purchase_orders.show'))
                            <a href="{{ route('purchasing.purchase_orders.show', $pr->converted_to_po_id) }}"
                                class="fw-semibold" style="font-size:.92rem;">
                                Lihat PO →
                            </a>
                        @endif
                        @if ($pr->converted_at)
    ...
    @if ($pr->isApproved() || $pr->isRejected() || $pr->isConverted())
        <div class="pr-info-card">
            <div class="card-body">
                ...
                <div class="d-flex flex-column gap-2">
                    ...
                    @if ($pr->isConverted() && $pr->converted_at)
                        <div class="timeline-item">
                            <div class="timeline-dot blue"></div>
                            <div style="font-size:.85rem;">
                                Diconvert ke PO
                                <span class="text-muted ms-1">{{ $pr->converted_at->format('d/m/Y H:i') }}</span>
                            </div>
                        </div>
                    @endif

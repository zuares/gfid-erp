...
class SyncReleasedSettlementTimesCommand extends Command
{
    ...
    public function handle(MarketplaceSyncService $syncService): int
    {
        ...
        $this->info(sprintf(
            ...
            Carbon::createFromTimestamp($from)->toDateString(),
            Carbon::createFromTimestamp($to)->toDateString(),
            $stores->count()
        ));
        ...
        foreach ($stores as $store) {
            ...
            try {
                ...
                $this->line(sprintf(
                    '%s: ditemukan=%d updated=%d tidak_terhubung=%d errors=%d',
                    $store->name,
                    $result['found'] ?? 0,
                    $result['updated'] ?? 0,
                    $result['unmatched'] ?? 0,
    ...
    private function resolveDateRange(): array
    {
        ...
        try {
            $from = Carbon::createFromFormat('!Y-m-d', (string) $fromOption);
            $to = Carbon::createFromFormat('!Y-m-d', (string) $toOption);
        } catch (\Throwable) {

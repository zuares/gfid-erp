...
<div class="page-wrap">
    <div class="ship-topbar">
        ...
        <div class="controls">
            ...
            <button class="btn btn-sm btn-ship-primary btn-pill" id="syncBtn" onclick="runSync()">Sync API</button>
...
if (scriptMatch) {
    ...
    scriptContent = scriptContent.replace(/gf-table-scroll/g, ''); // we wrapped it manually in blade
    ...
    // Fix innerHTML assignment where the div wrappers were
    scriptContent = scriptContent.replace(/<div class="gf-table-scroll">([\s\S]*?)<table/g, '<table');
    scriptContent = scriptContent.replace(/<\/table><\/div>/g, '</table>');
    
    // Fix class names in innerHTML
    scriptContent = scriptContent.replace(/prod-tab-loading/g, 'text-center text-muted py-4');
    scriptContent = scriptContent.replace(/<span class="prod-tab-spinner"><\/span>/g, '');

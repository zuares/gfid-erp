    ...
    function productHealth(product) {
        const profit = Number(product.operating_profit || 0);
        const margin = Number(product.margin_pct || 0);
        const hpp = Number(product.hpp || 0);
        if (hpp <= 0) return {key: 'missing_hpp', label: 'HPP belum lengkap', tone: 'warn', icon: 'bi-database-exclamation'};
        if (profit < 0) return {key: 'negative', label: 'Profit negatif', tone: 'bad', icon: 'bi-arrow-down-right'};
        if (margin < 10) return {key: 'thin_margin', label: 'Margin tipis', tone: 'warn', icon: 'bi-dash-circle'};
        return {key: 'healthy', label: 'Profit sehat', tone: 'good', icon: 'bi-check2'};
    }
    function productStatusMarkup(product) {
        const health = productHealth(product);
        return `<span class="an-product-status ${health.tone}"><i class="bi ${health.icon}"></i>${health.label}</span>`;
    }
    function renderProductInsights(rows) {
        const list = rows || [];
        const negative = list.filter(product => Number(product.operating_profit || 0) < 0).length;
        const missingHpp = list.filter(product => Number(product.hpp || 0) <= 0).length;
        ...
        $('anProductPulse').innerHTML = [
            ['Produk dianalisis', Number(list.length).toLocaleString('id-ID'), 'top 100 berdasarkan gross sales'],
            ['Gross sales eligible', money(totalGross), 'basis perbandingan portofolio'],
            ['Payout bersih', money(totalPayout), 'setelah fee settlement'],
            ['Profit operasional', money(totalProfit), `${totalGross > 0 ? (totalProfit / totalGross * 100).toFixed(1) : '0.0'}% dari gross sales`],
        ...
        const healthy = list.filter(product => productHealth(product).key === 'healthy').length;
        const coverage = list.length ? Math.round((list.length - missingHpp) / list.length * 100) : 0;
        $('anProductReading').innerHTML = `<i class="bi bi-info-circle"></i><span><strong>${healthy.toLocaleString('id-ID')} produk profit sehat</strong> · ${negative.toLocaleString('id-ID')} profit negatif · ${missingHpp.toLocaleString('id-ID')} belum punya HPP lengkap · coverage HPP ${coverage}%.</span>`;
        const focus = [];
        ...
        const bestMargin = [...list].filter(product => Number(product.gross_sales || 0) > 0).sort((a,b) => Number(b.margin_pct || 0) - Number(a.margin_pct || 0))[0];
        ...
        if (missingHpp) focus.push(['warn','bi-database-exclamation','Coverage HPP belum lengkap',`${missingHpp.toLocaleString('id-ID')} produk belum bisa dinilai penuh`,'Lengkapi master HPP']);
        if (bestMargin && bestMargin.product_key !== best?.product_key) focus.push(['','bi-graph-up-arrow','Margin operasional terbaik',`${esc(bestMargin.product_name)} · ${Number(bestMargin.margin_pct || 0).toFixed(1)}%`,'Jadikan benchmark']);
    ...
    function renderProductSummary(rows) {
        ...
        const healthFilter = $('anProductHealth')?.value || 'all';
        const filteredRows = (rows || []).filter(product => {
            const matchesQuery = !query || `${product.product_name} ${product.sku} ${product.product_key}`.toLowerCase().includes(query);
            const matchesHealth = healthFilter === 'all' || productHealth(product).key === healthFilter;
        ...
        $('anProductResultCount').textContent = `${filteredRows.length.toLocaleString('id-ID')} dari ${rows.length.toLocaleString('id-ID')} produk`;
        $('bestProductBody').innerHTML = sales.length ? sales.map((p,i) => `<tr><td class="an-product-value"><span class="an-rank ${i < 3 ? 'top' : ''}">${i+1}</span></td><td><span class="an-product"><span class="an-product-copy"><a class="an-product-link" href="${productUrl(p)}" title="Buka produk terkait"> <span class="an-product-name">${esc(p.product_name)}</span></a><span class="an-product-sku">Kode: ${esc(p.sku || '—')}</span></span></span></td><td>${Number(p.qty || 0).toLocaleString('id-ID')}</td><td class="an-product-value">${money(p.gross_sales)}<span class="an-table-subline">eligible gross sales</span></td><td class="an-product-value">${money(p.payout)}<span class="an-table-subline">setelah fee marketplace</span></td><td class="an-product-value">${money(p.hpp)}</td><td class="an-product-value" style="color:#b45309">${money(p.ad_cost)}<span class="an-table-subline">alokasi proporsional</span></td><td class="an-product-value" style="color:${p.gross_profit>=0?'#15803d':'#dc2626'}">${money(p.gross_profit)}<span class="an-table-subline">${Number(p.gross_sales || 0) > 0 ? (Number(p.gross_profit || 0) / Number(p.gross_sales) * 100).toFixed(1) : '0.0'}% gross sales</span></td><td class="an-product-value" style="color:${p.operating_profit>=0?'#15803d':'#dc2626'}">${money(p.operating_profit)}<span class="an-table-subline">${Number(p.margin_pct || 0).toFixed(1)}% margin</span></td><td>${productStatusMarkup(p)}</td></tr>`).join('') : '<tr><td colspan="10"><div class="an-empty">Tidak ada produk yang cocok dengan filter ini.</div></td></tr>';

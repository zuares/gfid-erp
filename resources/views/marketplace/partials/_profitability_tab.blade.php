...
@php
    ...
    */
    ...
    // ── Group by KATEGORI ──
    // GMV Max Auto adalah agregat campaign/produk, bukan kategori SKU regular,
    // sehingga hanya ditampilkan pada tab GMV Max Auto tersendiri.
    $byCategory = $displayRows
        ->groupBy(function ($r) {
            return $r->camp->item_category ?? 'Belum termapping';
        })
                ...
                @forelse($byCategory as $index => $cat)
                    <tr style="border-bottom: 1px solid var(--dsh-border); {{ $cat->unknownCount > 0 && $cat->profit == 0 ? 'background: rgba(217, 119, 6, 0.045);' : ($cat->profit >= 0 ? '' : 'background: rgba(220, 38, 38, 0.045);') }}">
                        ...
                        <td style="padding:.6rem .5rem;">
                            <span style="font-weight:700; color:{{ $cat->unmapped ? '#b45309' : 'var(--text)' }}; font-size:.85rem;">
                                {{ $cat->name }}
                            </span>
                            @if($cat->unmapped)

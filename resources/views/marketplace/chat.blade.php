...
(function () {
    ...
    setInterval(async () => {
        ...
        if (Date.now() - lastPoll >= interval) {
            ...
            // Simpan jumlah unread sebelum sync
            totalUnreadBefore = conversations.reduce((sum, c) => sum + (c.unread_count || 0), 0);
            
            // Panggil API dengan sync=true jika websocket putus agar selalu update dari Shopee
            await loadConversations(!echoConnected);
            
            // Cek apakah ada penambahan unread_count
            let totalUnreadAfter = conversations.reduce((sum, c) => sum + (c.unread_count || 0), 0);
            if (totalUnreadAfter > totalUnreadBefore) {
                playSound();
            }

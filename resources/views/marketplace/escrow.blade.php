...
(() => {
    ...
    const groupedFields = (data, groups, orderSn, storeId) => {
        ...
        const renderGroup = (title, fields) => `<div class="escrow-income-group"><h4>${escapeHtml(title)} <span class="escrow-income-group-count">${fields.length} field</span></h4><div class="escrow-income-grid">${fields.map(([key, value]) => key === 'items' ? `<div class="escrow-items-field"><div class="escrow-items-field-label">${escapeHtml(fieldLabel(key))}</div>${renderItemDetails(value, orderSn, storeId)}</div>` : `<div class="escrow-income-row"><span>${escapeHtml(fieldLabel(key))}</span><span>${detailValue(key, value, orderSn, storeId)}</span></div>`).join('')}</div></div>`;
    ...
    const renderItemDetails = (items, orderSn, storeId) => {
        if (!Array.isArray(items) || !items.length) return '<span class="escrow-muted">Tidak ada item pada response Shopee.</span>';
        const cards = items.map((item, index) => {
            const itemName = item?.item_name || `Item ${index + 1}`;
            const fields = Object.entries(item || {}).map(([key, value]) => `<div class="escrow-income-row"><span>${escapeHtml(key)}</span><span>${detailValue(key, value, orderSn, storeId)}</span></div>`).join('');
            return `<div class="escrow-item-card"><div class="escrow-item-title">${escapeHtml(itemName)}</div><div class="escrow-income-grid">${fields || '<span class="escrow-muted">Tidak ada detail item.</span>'}</div></div>`;
        }).join('');
        return `<div class="escrow-items-list">${cards}</div><div class="escrow-item-raw">${detailValue('items', items, orderSn, storeId)}</div>`;
    };

...
.shp-suggest {
    ...
    z-index: 400; max-height: 108px; overflow-y: auto;
    background: var(--card, #fff); border: 1px solid rgba(148,163,184,.28);
    border-radius: 8px; box-shadow: 0 6px 18px rgba(15,23,42,.1);
    padding: .18rem; scrollbar-width: thin;
}
body[data-theme="dark"] .shp-suggest { background: #0f172a; border-color: rgba(51,65,85,.8); box-shadow: 0 8px 22px rgba(0,0,0,.5); }
.shp-suggest.is-open { display: block; }
.shp-suggest-item {
    display: flex; align-items: baseline; gap: .5rem;
    padding: .32rem .5rem; border-radius: 6px; cursor: pointer; line-height: 1.2;
}
.shp-suggest-item:hover, .shp-suggest-item.is-active { background: rgba(148,163,184,.14); }
body[data-theme="dark"] .shp-suggest-item:hover, body[data-theme="dark"] .shp-suggest-item.is-active { background: rgba(148,163,184,.16); }
.shp-suggest-code { font-family: monospace; font-weight: 700; font-size: .8rem; letter-spacing: 0; white-space: nowrap; color: #334155; }
body[data-theme="dark"] .shp-suggest-code { color: #e2e8f0; }
.shp-suggest-name { font-size: .74rem; color: #64748b; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
body[data-theme="dark"] .shp-suggest-name { color: #94a3b8; }
.shp-suggest-cat { margin-left: auto; font-size: .64rem; color: #94a3b8; white-space: nowrap; flex-shrink: 0; }
.shp-suggest-empty { padding: .42rem .5rem; font-size: .76rem; color: #94a3b8; }
.shp-suggest mark { background: rgba(250,204,21,.4); color: inherit; padding: 0; border-radius: 2px; }

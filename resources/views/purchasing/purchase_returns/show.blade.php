...
@extends('layouts.app')
@section('title', 'Purchase Return ' . ($ret->code ?? ''))
  ...
  <div class="page-wrap">
    ...
    <div class="page-header d-flex justify-content-between align-items-start mb-3 gap-2">
      <div class="d-flex flex-column gap-1">
        <div class="d-flex flex-wrap align-items-center gap-2">
          <h1 class="mb-0 page-header-title">Purchase Return</h1>
      ...
      <div class="page-header-actions d-flex align-items-center gap-2">
        <a href="{{ route('purchasing.purchase_receipts.show', $ret->purchase_receipt_id ?? $ret->grn_id ?? ($ret->grn?->id ?? 0)) }}"
    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-12 col-lg-6">
        <div class="card-soft h-100">
          <div class="p-3 p-sm-4">
            <h6 class="section-title mb-3">Edit Draft</h6>
            ...
            <div class="text-muted small mt-2">
              * Return mengurangi stok di gudang GRN, dan jurnal reversal mengikuti jurnal GRN (pro-rata).
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="p-2 p-sm-3">
            <form method="POST" action="{{ route('purchasing.purchase_returns.update', $ret->id) }}">
                  ...
                  <tbody class="small">
                    @foreach($ret->lines as $i => $ln)
                      @php
                        ...
                        $rem = (float)($remaining[(int)$ln->purchase_receipt_line_id] ?? 0);
                        ...
                        <td data-label="Item">
                          ...
                          @if($isDraft && !$isVoided)
                            ...
                            <input type="text"
                              ...
                              placeholder="notes line (optional)"
                              value="{{ old("lines.$i.notes", $ln->notes) }}">
                          @endif
                        </td>

                        <td class="text-end mono" data-label="Received">
                          {{ rtrim(rtrim(number_format($received, 4, ',', '.'), '0'), ',') }}
                        </td>

                        <td class="text-end mono" data-label="Remaining">
              ...
              @if($isDraft && !$isVoided)
                <div class="d-flex justify-content-end gap-2 mt-3">
                  <button class="btn btn-primary btn-sm btn-pill" type="submit">
                    <i class="bi bi-save2 me-1"></i> Save Draft (Lines)
                  </button>
                </div>
              @endif
            </form>
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
              <div class="text-muted small">
                * POST akan mengurangi stok & membuat jurnal return. VOID akan membalik stok & jurnal return.
              </div>

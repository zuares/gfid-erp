...
@section('title', 'Scan Shipment • ' . $shipment->code)
...
@push('head')
    <style>
        ...
        body[data-theme="light"] .page-wrap.page-theme-shopee { background: #fff7ed; }
        body[data-theme="light"] .page-wrap.page-theme-tiktok { background: #ecfeff; }
        ...
        .page-theme-shopee .store-badge { border-color: #f97316; color: #9a3412; }
        .page-theme-tiktok .store-badge { border-color: #06b6d4; color: #0f766e; }
        ...
        .btn-theme-main { transition: background .12s ease, border-color .12s ease, color .12s ease, box-shadow .12s ease; }
        .btn-theme-outline { background: transparent; transition: background .12s ease, border-color .12s ease, color .12s ease; }
        ...
        .page-theme-default .btn-theme-main:hover { background: #1d4ed8; border-color: #1d4ed8; }
        .page-theme-default .btn-theme-outline { border-color: rgba(148, 163, 184, 0.7); color: #4b5563; }
        .page-theme-default .btn-theme-outline:hover { background: rgba(226, 232, 240, 0.7); }
        ...
        .page-theme-shopee .btn-theme-main:hover { background: #ea580c; border-color: #ea580c; }
        .page-theme-shopee .btn-theme-outline { border-color: rgba(248, 113, 113, 0.9); color: #b91c1c; }
        .page-theme-shopee .btn-theme-outline:hover { background: rgba(254, 226, 226, 0.9); }
        ...
        .page-theme-tiktok .btn-theme-main:hover { background: #0e7490; border-color: #0e7490; }
        .page-theme-tiktok .btn-theme-outline { border-color: rgba(45, 212, 191, 0.8); color: #0f766e; }
        .page-theme-tiktok .btn-theme-outline:hover { background: rgba(224, 242, 254, 0.9); }

        body[data-theme="dark"] .btn-theme-outline { color: #e5e7eb; border-color: rgba(148, 163, 184, 0.6); }
        body[data-theme="dark"] .btn-theme-outline:hover { background: rgba(15, 23, 42, 0.9); }
        ...
        body[data-theme="dark"] .scan-input::placeholder { color: #6b7280; }
        .scan-input:focus {
        ...
        body[data-theme="dark"] .info-label { color: #6b7280; }
        ...
        @media (min-width: 768px) {
            ...
            body[data-theme="dark"] .info-main-col { border-left-color: rgba(51, 65, 85, 0.85); }
        }
        ...
        .lines-wrapper::-webkit-scrollbar { width: 6px; }
        .lines-wrapper::-webkit-scrollbar-track { background: transparent; }
        .lines-wrapper::-webkit-scrollbar-thumb { background: rgba(148, 163, 184, 0.7); border-radius: 999px; }

        @media (max-width: 768px) {
            .lines-wrapper { max-height: 320px; }
        }

        .table-lines { margin-bottom: 0; }
        ...
        body[data-theme="dark"] .table-lines tbody td { border-top-color: rgba(51, 65, 85, 0.85); }

        .table-lines tbody tr:nth-child(even) { background-color: rgba(249, 250, 251, 0.86); }
        body[data-theme="dark"] .table-lines tbody tr:nth-child(even) { background-color: rgba(15, 23, 42, 0.96); }

        .item-code { font-weight: 600; font-size: .9rem; }
        body[data-theme="dark"] .item-code { color: #e5e7eb; }
        ...
        .qty-edit-form { display: inline-flex; align-items: center; gap: .25rem; }
        .qty-edit-input { width: 80px; text-align: right; padding-right: .4rem; }
        .qty-edit-save-btn { border-radius: 999px; padding-inline: .4rem; padding-block: .15rem; font-size: .75rem; line-height: 1; }

        /* ========= LAST SCANNED ========= */
        .last-scanned-row td { background: rgba(254, 243, 199, 0.96) !important; }
        .last-scanned-row td:first-child { border-left: 3px solid rgba(234, 88, 12, 0.9); }

        body[data-theme="dark"] .last-scanned-row td { background: rgba(30, 64, 175, 0.70) !important; }
        ...
        .last-scanned-pulse { animation: rowPulseSoft 0.9s ease-out 1; }
        ...
        .btn-jump-last::before { content: '⤵'; font-size: .8rem; }
        .btn-jump-last:hover { background: rgba(219, 234, 254, 0.96); }
        ...
        .btn-jump-last-highlight { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.65); }
    </style>
@endpush
...
@section('content')
    @php
        ...
        // importPreview & importPreviewSummary sudah dikirim dari controller edit()
    @endphp
                    ...
                    <div class="col-md-5">
                        <div class="scan-panel h-100 d-flex flex-column justify-content-between">
                            <form id="scanForm" method="POST" action="{{ route('sales.shipments.scan_item', $shipment) }}">
                                @csrf
                                <div class="mb-2">
                                    {{-- HILANGKAN TEXT PETUNJUK --}}
                                    <input type="text" name="scan_code" class="form-control scan-input" id="scanInput"
                                        placeholder="Scan..." autocomplete="off" required>
                                </div>
                            </form>
                        </div>
                    </div>

                    {{-- KANAN: CHANNEL + CATATAN + TOMBOL KE SCAN TERAKHIR (HIDE DI MOBILE) --}}
                    <div class="col-md-7 info-main-col d-none d-md-block">
                        {{-- Channel / Store --}}
                        <div class="mb-3">
        ...
        <div class="card card-main">
            <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap gap-2 align-items-center">
                        ...
                        <div class="ms-2">
                            <div class="input-group input-group-sm" style="width: 230px;">
                                ...
                                <input type="text" id="itemFilterInput" class="form-control" placeholder="Kode / nama barang">
                            </div>
                        </div>
                    </div>
                </div>

                {{-- IMPORT & CLEAR (hanya draft) --}}
                <div class="mb-3">
                    <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
                        ...
                        {{-- Tombol bersihkan semua baris --}}
                        @if ($shipment->lines->isNotEmpty())
                            <form method="POST" action="{{ route('sales.shipments.clear_lines', $shipment) }}"
                        ...
                        <tbody id="linesTbody">
                            @forelse ($shipment->lines as $line)
                                <tr class="{{ $lastScannedLineId == $line->id ? 'last-scanned-row' : '' }}"
                                    ...
                                    {{-- HIDE NAMA BARANG DI MOBILE (TIDAK DOUBLE) --}}
                                    ...
                                    <td class="text-end">
                                        ...
                                        <form action="{{ route('sales.shipments.update_line_qty', $line) }}"
                                            ...
                                            @csrf
                                            @method('PATCH')
                                            <input type="number" name="qty" class="form-control form-control-sm qty-edit-input"
                                                min="0" value="{{ $line->qty_scanned }}">
                                            <button type="submit" class="btn btn-primary btn-sm qty-edit-save-btn">✔</button>
                                        </form>
                                    </td>
                                    <td class="text-end">
                                        <form action="{{ route('sales.shipments.destroy_line', $line) }}" method="POST"
                                            class="d-inline js-delete-line-form" data-line-id="{{ $line->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger" title="Hapus baris">
                                                🗑
                                            </button>
                                        </form>
                                    </td>
                ...
                <div class="mt-3 d-flex flex-wrap justify-content-between align-items-center small text-muted gap-2">
                    ...
                    <div class="d-flex flex-wrap gap-2 align-items-center">
                        {{-- Kembali ke detail --}}
                        <a href="{{ route('sales.shipments.show', $shipment) }}" class="btn btn-theme-outline">
                            &larr; Kembali ke detail
                        </a>

                        {{-- Import barang dari file (auto preview) --}}
                        <form id="importPreviewForm" method="POST"
                            action="{{ route('sales.shipments.import_preview', $shipment) }}"
                            enctype="multipart/form-data" class="d-inline">
                            @csrf
                            <input id="importFileInput" type="file" name="file"
                                class="d-none @error('file') is-invalid @enderror"
                                accept=".csv,.txt,.xlsx,.xls" required>
                            <button type="button" class="btn btn-theme-outline"
                        ...
                        {{-- EXPORT: HIDE DI MOBILE --}}
                        @if ($shipment->lines->isNotEmpty())
                            <a href="{{ route('sales.shipments.export_lines', $shipment) }}"
                                class="btn btn-theme-outline d-none d-md-inline-flex">
                                Export Barang (CSV)
                            </a>
                        @endif
                        ...
                        @if ($importPreviewSummary)
                            <div class="d-flex flex-wrap gap-2 mb-3">
                                <span class="summary-pill">
                                    OK:
                                    <span class="fw-semibold ms-1">{{ $importPreviewSummary['ok_count'] ?? 0 }}</span>
                                </span>
                                <span class="summary-pill">
                                    Dilewati:
                                    <span class="fw-semibold ms-1">{{ $importPreviewSummary['skip_count'] ?? 0 }}</span>
                                </span>
                                <span class="summary-pill">
                                    Total Qty OK:
                        ...
                        <form method="POST" action="{{ route('sales.shipments.import_lines', $shipment) }}">
                            ...
                            @foreach ($importPreview as $i => $row)
                                @if (($row['status'] ?? null) === 'ok')
                                    <input type="hidden" name="rows[{{ $i }}][product_code]" value="{{ $row['item_code'] }}">
                                    <input type="hidden" name="rows[{{ $i }}][qty]" value="{{ (int) ($row['parsed_qty'] ?? 0) }}">
                                @endif
                            @endforeach
...
@push('scripts')
    <script>
        (function() {
            console.log('GFID shipment EDIT scan script loaded');
            ...
            // === auto submit import preview saat pilih file ===
            ...
            function playSuccessBeep() { playBeep(1046); }
            function playErrorBeep() { playBeep(220, 0.18, 0.25); }
            ...
            function scrollToRow(lineId, withPulse = true) {
                ...
                for (let i = 0; i < prev.length; i++) prev[i].classList.remove('last-scanned-row');
                ...
                linesWrapper.scrollTo({ top: Math.max(targetScrollTop, 0), behavior: 'smooth' });
            ...
            function bindQtyClickForRow(row) {
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    fetch(form.action, {
                        ...
                        let data = null;
                        try { data = await res.json(); } catch (err) { form.submit(); return; }
                        ...
                        if (data.deleted) {
                            ...
                            const qtyVal = typeof data.qty !== 'undefined' ? data.qty : (Number(input.value) || 0);
            ...
            function bindDeleteForm(form) {
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    fetch(form.action, {
                        ...
                        let data = null;
                        try { data = await res.json(); } catch (e) { form.submit(); return; }
            ...
            if (linesTbody) {
                const rows = linesTbody.querySelectorAll('tr[data-line-id]');
                for (let i = 0; i < rows.length; i++) bindQtyClickForRow(rows[i]);

                const deleteForms = linesTbody.querySelectorAll('.js-delete-line-form');
                deleteForms.forEach(form => bindDeleteForm(form));
            }

            // flag: apakah perlu auto-show modal preview (hanya ketika habis upload)
            const shouldAutoShowPreview = {{ request()->boolean('show_preview', false) ? 'true' : 'false' }};

            // Fokus & scroll awal
            window.addEventListener('load', function() {
                if (isDraft && scanInput) focusScan();
                ...
                if (lastScannedLineId) {
                    ...
                    if (btnJumpLast) {
                        ...
                        setTimeout(() => { btnJumpLast.classList.remove('btn-jump-last-highlight'); }, 2200);
                    }
                }

                // Auto show modal preview import HANYA kalau ada flag show_preview=1
                const importPreviewModalEl = document.getElementById('importPreviewModal');
                if (importPreviewModalEl && shouldAutoShowPreview && window.bootstrap && typeof window.bootstrap.Modal === 'function') {
            ...
            // Filter cepat (tetap jalan walau header filter disembunyikan di mobile)
            if (itemFilterInput && linesWrapper) {
                itemFilterInput.addEventListener('input', function() {
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        if (!term || codeText.includes(term) || nameText.includes(term)) row.style.display = '';
            ...
            if (scanForm && scanInput && linesTbody) {
                scanForm.addEventListener('submit', function(e) {
                    ...
                    fetch(scanForm.action, {
                        ...
                        let data = null;
                        try { data = await res.json(); } catch (err) { scanForm.submit(); return; }
                        ...
                        if (!line || !line.id) {
                            ...
                            if (!row) {
                                ...
                                row.innerHTML =
                                    ...
                                    // Nama barang ada, tapi hidden di mobile via class d-none d-md-table-cell (TIDAK DOUBLE)
                                    '<td class="d-none d-md-table-cell">' +
                                    ...
                                    '<td class="text-end">' +
                                        ...
                                        '<form action="' + updateUrl + '" method="POST" class="d-inline qty-edit-form d-none" data-line-id="' + line.id + '">' +
                                    ...
                                    '</td>' +
                                    '<td class="text-end">' +
                                        '<form action="' + deleteUrl + '" method="POST" class="d-inline js-delete-line-form" data-line-id="' + line.id + '">' +
                                            '<input type="hidden" name="_token" value="{{ csrf_token() }}">' +
                            ...
                            if (qtyEl) qtyEl.textContent = new Intl.NumberFormat('id-ID').format(line.qty_scanned || 0);
                            ...
                            if (typeof totals.total_lines !== 'undefined' && summaryTotalLines) summaryTotalLines.textContent = totals.total_lines;
                            if (typeof totals.total_qty !== 'undefined') {
                                const f = new Intl.NumberFormat('id-ID').format(totals.total_qty);
                                if (summaryTotalQty) summaryTotalQty.textContent = f;
                                if (footerTotalQty) footerTotalQty.textContent = f;

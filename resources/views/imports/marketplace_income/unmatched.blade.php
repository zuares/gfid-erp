{{-- resources/views/imports/marketplace_income/unmatched.blade.php --}}
@extends('layouts.app')
@section('title','Unmatched • Marketplace Income')
...
@php
  ...
  $kpi = $kpi ?? (object)[
    ...
    'fee_sum' => 0,
    'refund_sum' => 0,
    'released_date_min' => null,
    'released_date_max' => null,
  ];
...
<style>
  ...
  .kpi-grid{ display:grid; grid-template-columns: repeat(4, 1fr); gap:.6rem; }
...
<div class="page">
  ...
  <div class="d-flex justify-content-between align-items-start gap-2 mb-3">
    <div>
      ...
      <div class="text-muted small d-flex flex-wrap gap-1 align-items-center">
        ...
        <span class="chip badge-warn">unmatched</span>
        @if($filterActive) <span class="chip badge-warn">filtered</span> @endif
      </div>

      <div class="tabs mt-2">
        <a class="tabx" href="{{ $showUrl }}">All</a>
        <a class="tabx" href="{{ $matchedUrl }}">Matched</a>
        <a class="tabx active" href="{{ $unmatchedUrl }}">Unmatched</a>
      </div>
    </div>
  ...
  <div class="cardx p-3 mb-3">
    <div class="kpi-grid">
      <div class="cardx kpi-card">
        ...
        <div class="kpi">{{ number_format((int)($kpi->orders_count ?? 0)) }}</div>
      </div>
      <div class="cardx kpi-card">
        <div class="kpi-label">Net</div>
        <div class="kpi">{{ $money($kpi->net_sum ?? 0) }}</div>
      </div>
      <div class="cardx kpi-card">
        <div class="kpi-label">Fee</div>
        <div class="kpi">{{ $money($kpi->fee_sum ?? 0) }}</div>
      </div>
      <div class="cardx kpi-card">
        <div class="kpi-label">Refund</div>
        <div class="kpi">{{ $money($kpi->refund_sum ?? 0) }}</div>
      </div>
    </div>
    <div class="muted small mt-2 mono">
      {{ $kpi->released_date_min ?? '-' }} → {{ $kpi->released_date_max ?? '-' }}
    </div>
  </div>

  {{-- FILTER (minimal) --}}
  <form method="GET" action="{{ route('imports.marketplace_income.unmatched', ['batch' => $batch]) }}" class="cardx p-3 mb-3">
    <div class="filter-grid">
      ...
      <div class="d-flex gap-2">
        ...
        <a class="btn btn-outline-secondary btn-sm px-3" style="height:40px;" href="{{ route('imports.marketplace_income.unmatched', ['batch' => $batch]) }}">Reset</a>
      </div>
    </div>
  </form>
  ...
  <div class="cardx">
    <div class="p-3 d-flex justify-content-between align-items-center">
      <div class="fw-bold">Unmatched Orders</div>
      ...
      <table class="tbl">
        <thead>
          <tr>
            ...
            <th class="text-end" style="width:13%">Net</th>
            <th class="text-end" style="width:13%">Fee</th>
            <th class="text-end" style="width:12%">Refund</th>
          </tr>
        </thead>
        <tbody>
          @forelse($items as $it)
          ...
          @empty
            <tr><td colspan="6" class="text-muted p-3">No data.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
